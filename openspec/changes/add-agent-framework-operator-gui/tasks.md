...
- [x] 2.2a Ticket 01 implementation: GitHub intake, immutable redacted submission, current authorization and GUI readback without starting a run; direct local/live-provider evidence in issue-01-evidence.md.
- [ ] 2.2b Ticket 01 distributed acceptance: repeat GUI intake/readback and service/client restart with the server on a separate machine; waiting for a reachable test server.
