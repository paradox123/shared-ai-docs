...
#### Scenario: Restored control ownership cannot be transferred
- **WHEN** a dossier retains a holder and a pending transfer request and either
  contributor observes or submits a control action against the restored run
- **THEN** every advertised control capability is disabled, transfer and forced
  takeover are rejected as read-only, and historical ownership and checksums remain unchanged
