...
- Readiness Status: ORCHESTRATION COMPLETE; `MD-E2E-1` and `MD-E2E-2` are `ACCEPTED`; `MD-E2E-3` is `IMPLEMENTATION READY`; `MD-E2E-4` still needs hardening; `MD-E2E-5` is deferred.
...
|---|---|---|---|
| `MD-PR1` | `MD-E2E-1`, `MD-E2E-3` | ready_for_delivery | Mock fixture policy and validator are accepted in MD-E2E-1; MD-E2E-3 is hardened for standard-gate enforcement. |
...
| `MD-PR7` | `MD-E2E-3` | ready_for_delivery | Hardening froze the exact removal/replacement strategy; implementation must remove KI-fuer-KMU from `run-contract-checks.sh`/`setup-fixture.sh` standard defaults. |
...
| complete | `MD-E2E-3` | Hardened and implementation-ready. | Standard command is `run-mock-e2e-checks.sh all --keep`; `run-contract-checks.sh all` may only remain as mock-only shim or non-leading legacy command; no KI-fuer-KMU compatibility fixture is allowed. |
...
| `MD-E2E-3` implementation | serial after hardening | Ready for one `spec-change-delivery` session; do not parallelize with MD-E2E-4 final docs sync because README command references are shared. | `run-contract-checks.sh`, `setup-fixture.sh`, `run-mock-e2e-checks.sh`, README; accepted mock data and runner internals remain read-only. | MD-E2E-3 delivery/session owner. |
...
1. Implement `MD-E2E-3` with `spec-change-delivery`, replacing/deactivating legacy standard gates now that `run-mock-e2e-checks.sh all --keep` exists and has accepted evidence.
2. Harden then implement `MD-E2E-4` for final README, parent, OpenSpec and evidence sync.
3. Consider `MD-E2E-5` only after the local mock E2E baseline and standard-gate migration are accepted.
...
`MD-E2E-2` is accepted and archived; closeout evidence exists under `tests/docworkflow-agent-delivery/e2e/evidence/closeout-md-e2e-2-*` and `openspec/changes/archive/2026-05-09-docworkflow-agent-mock-e2e-md-e2e-2-local-runner/implementation-evidence.md`. `MD-E2E-3` is hardened, queued and ready for `spec-change-delivery`. `MD-E2E-4` still requires hardening after standard-gate migration evidence is stable. `MD-E2E-5` remains deferred.

## Mini-Retro

- Was wurde entschieden? `MD-E2E-1` and `MD-E2E-2` are accepted; the local mock runner is the accepted baseline for later standard-gate migration.
- Was wurde geaendert? Child Index, Coverage Matrix, hardening queue and evidence pointers now reference the archived `MD-E2E-2` OpenSpec change and retained closeout evidence.
- Was bleibt offen? `MD-E2E-3` needs implementation/closeout; `MD-E2E-4` still needs child-spec-hardening; `MD-E2E-5` remains deferred.
- Welche Evidenz/Verification fehlt? `MD-E2E-3` closeout evidence after script/README migration.
...
- Session-/Kontextzustand: Start `spec-change-delivery` for `MD-E2E-3` from `_specs/child-session-handoffs/md-e2e-3-session-handoff.md`.
