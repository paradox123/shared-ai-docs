**Date:** 2026-05-10
**Status:** 🟡 Spec
**Scope:** Parent/Child orchestration pack for `_specs/2026-05-09 Agent Delivery Visible Codex App Sessions.md`. Tracks child delivery status; no live MD-E2E-5 execution in orchestration or hardening sessions.
...
- Goldstandard Status: active control pack; S1 is delivered, S2/S4/S5 are accepted, and S3 is implementation-ready for its live runner delivery gate.
...
- Out of Scope for S2 delivery sync: Launcher code edits, test runner edits, live `MD-E2E-5` execution, `codex app-server` session launch, `codex exec` launch, app-server archive execution.
...
- Readiness Status: `ADV-CAS-S1` is `IMPLEMENTED`; `ADV-CAS-S2` is `ACCEPTED`; `ADV-CAS-S4` is `ACCEPTED`; `ADV-CAS-S5` is `ACCEPTED`; `ADV-CAS-S3` is `IMPLEMENTATION READY`. No `MD-E2E-5` live run has been executed yet; the live run is authorized only for the next S3 `spec-change-delivery` control workflow.
...
| ADV-CAS-S2 | `_specs/2026-05-10 Agent Delivery Visible Codex App Sessions ADV-CAS-S2 Visible Evidence Validator.md` | `ADV-PR1`, `ADV-PR5`, `ADV-PR6`; supports `ADV-PR10` | `IMPLEMENTATION READY` | `child-session-handoffs/adv-cas-s2-session-handoff.md` | `not_used`: no active `agent-delivery-visible-session-validator` change exists; proposed ledger was never created | S1 implemented; accepted S2 validator now unblocks S5/S3 validation use | `_specs/2026-05-10 Agent Delivery Visible Codex App Sessions ADV-CAS-S2 Visible Evidence Validator.md`; `_specs/2026-05-10 Agent Delivery Visible Codex App Sessions Orchestration Pack.md`; `_specs/child-session-handoffs/adv-cas-s2-session-handoff.md`; `skills-repo/tools/ValidateAgentDeliveryLaunchEvidence.cs`; `skills-repo/tools/ValidateVisibleCodexAppSessionEvidence.cs`; `tests/docworkflow-agent-delivery/e2e/fixtures/visible-app-session-evidence/**`; `tests/docworkflow-agent-delivery/e2e/validators/visible-app-session-evidence.js` | Closeout replay ran: `ValidateAgentDeliveryLaunchEvidence.cs --help`; S1 visible fixture validator; `ValidateChildReadiness.cs`; `ValidateVisibleCodexAppSessionEvidence.cs --fixture`; setup-error fixture exit-code check; existing launch-evidence fixture regression; source `rg` check; `git diff --check`. `MD-E2E-5` was not run | Closeout accepted. Retained S2 validator fixture summary `RESULT: PASS (11 cases)`; setup-error fixture exits `2`; existing launch-evidence regression `RESULT: PASS (10 cases)`; no S2 `visible_codex_app_session` launch evidence found, so `archive_status: no_s2_visible_thread_created` | If S1 delivery changes evidence names, re-enter S2 hardening before S5/S3 | Start fresh `spec-change-delivery` for ADV-CAS-S2 only if S2 evidence must be replayed; otherwise continue with dependent S5/S3 work |
| ADV-CAS-S3 | `_specs/2026-05-10 Agent Delivery Visible Codex App Sessions ADV-CAS-S3 MD-E2E-5 Integration.md` | `ADV-PR7`; consumes `ADV-PR2`, `ADV-PR5`, `ADV-PR6`, `ADV-PR8`, `ADV-PR9`, `ADV-PR10` | `IMPLEMENTATION READY`; serialized integration child hardened for live `MD-E2E-5` runner delivery after S1 implemented and S2/S4/S5 accepted | `child-session-handoffs/adv-cas-s3-session-handoff.md` | Proposed: `openspec/changes/agent-delivery-visible-md-e2e-5-suite/`; no S3 ledger created during hardening | Depends on S1 app-server adapter, S2 visible evidence validator, S4 control-boundary validator and S5 closeout archive support; must not replace mock-only standard gate | `_specs/2026-05-10 Agent Delivery Visible Codex App Sessions ADV-CAS-S3 MD-E2E-5 Integration.md`; `_specs/2026-05-10 Agent Delivery Visible Codex App Sessions Orchestration Pack.md`; `_specs/child-session-handoffs/adv-cas-s3-session-handoff.md`; `tests/docworkflow-agent-delivery/testcases/md-e2e-5-visible-codex-app-sessions.md`; `tests/docworkflow-agent-delivery/scripts/run-visible-app-session-workflow-checks.sh`; `tests/docworkflow-agent-delivery/README.md`; `tests/docworkflow-agent-delivery/e2e/session-workflow-live/**`; `tests/docworkflow-agent-delivery/e2e/evidence/*visible-app*` | Hardening/readiness: `bash -n tests/docworkflow-agent-delivery/scripts/run-visible-app-session-workflow-checks.sh`; `tests/docworkflow-agent-delivery/scripts/run-visible-app-session-workflow-checks.sh control-boundary`; S4 control-boundary fixture replay; S2 visible evidence fixture replay; S5 archive fixture replay; `ValidateChildReadiness.cs`; embedded JSON parse; `git diff --check`. Delivery gate: `tests/docworkflow-agent-delivery/scripts/run-visible-app-session-workflow-checks.sh --run-id <id> --keep` in a later launcher/control workflow | Runner implemented; retained not-ready rehearsal at `tests/docworkflow-agent-delivery/e2e/session-workflow-live/20260511T103700Z-adv-cas-s3-rehearsal/visible-session-summary.json`. No passing live `MD-E2E-5` evidence exists yet. Delivery closeout must retain `visible-session-summary.json`, six visible evidence records, S2 validator output, S4 control-boundary status, S5 archive/no-thread status and exact final output proof for `1\n2\n3\n4\n5\n` | If S3 implementation exposes a required change to S1/S2/S4/S5 owned tools, stop with `NOT READY` and route a follow-up to that owner before accepting S3 | Continue `spec-change-delivery` for ADV-CAS-S3 with a live `MD-E2E-5` delivery/control workflow and real parent plus five child visible-session evidence before accepting S3 |
...
| `ADV-PR10` | S1/S2/S5 plus docs sync in S3/S5 | partial | S5 synced closeout archive wording; S3 may still add runner-specific docs. |
...
| complete | `ADV-CAS-S3` | Hardened MD-E2E-5 suite integration after S1/S2/S4/S5 contracts. | Exact runner summary schema, opt-in gate semantics and implementation handoff are ready; live execution remains delivery-only. |
...
| S3 delivery | implementation | Serialized after accepted downstream prerequisites because it integrates final live workflow. | Mock runner baseline remains read-only and must not be replaced. | S3 integration owner. |

## Recommended Execution Order

1. S1 is implemented; S2, S4 and S5 are accepted downstream prerequisites.
2. Start a fresh `spec-change-delivery` session for exactly `ADV-CAS-S3`.
3. Run the live `MD-E2E-5` regression only inside that S3 delivery/control workflow and retain evidence.
...
- Acceptance Signal: Later control/launcher run passes only when final output, S2 visible evidence, S4 control boundary and S5 archive/no-thread evidence all pass.
- Next Skill: `spec-change-delivery` for exactly `ADV-CAS-S3`.
...
No S3 Agent Delivery Session Launch/Queue Evidence was created in this hardening session.

Reason: S3 hardening is a handoff boundary and live visible-session execution is out of scope here. `_specs/child-session-handoffs/adv-cas-s3-session-handoff.md` records `manual_start_required` for the next fresh `spec-change-delivery` session. That session may create real launch/queue evidence and then run `tests/docworkflow-agent-delivery/scripts/run-visible-app-session-workflow-checks.sh --run-id <id> --keep`.

## Mini-Retro

- Was wurde entschieden? S3 is the serialized implementation-ready integration child for live `MD-E2E-5`.
- Was wurde geaendert? S3 hardening synchronized the Child Index, S3 handoff, child spec, testcase contract and README live-gate wording. No live test was run.
- Was bleibt offen? The later S3 `spec-change-delivery` implementation and live launcher/control regression.
- Welche Evidenz/Verification fehlt? No runtime visible-app launch evidence exists yet; no S3 launch/queue evidence was created in hardening; no live `MD-E2E-5` run by design.
- Welche Skill-/Workflow-Reibung ist aufgefallen? The hardening boundary can only rehearse accepted fixture gates; the implementation session must create the real live evidence before any accepted pass claim.
- Session-/Kontextzustand: Continue with a fresh `spec-change-delivery` session for `ADV-CAS-S3`.
