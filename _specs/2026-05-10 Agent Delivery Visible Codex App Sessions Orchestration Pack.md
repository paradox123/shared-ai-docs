**Date:** 2026-05-10
**Status:** 🟡 Spec
**Scope:** Parent/Child orchestration pack for `_specs/2026-05-09 Agent Delivery Visible Codex App Sessions.md`. Tracks child delivery status; no live MD-E2E-5 execution in S2.
...
- Goldstandard Status: active control pack; S1 is delivered, S2 is accepted, and remaining children still require their own hardening/delivery gates.
...
- Readiness Status: `ADV-CAS-S1` is `IMPLEMENTED`; `ADV-CAS-S2` is `ACCEPTED`; `ADV-CAS-S4` and `ADV-CAS-S5` are `IMPLEMENTATION READY`; `ADV-CAS-S3` still requires its own gates. No `MD-E2E-5` live run is authorized by S2/S5 hardening.
...
| ADV-CAS-S5 | `_specs/2026-05-10 Agent Delivery Visible Codex App Sessions ADV-CAS-S5 Closeout Archive Support.md` | `ADV-PR9`; supports `ADV-PR5`, `ADV-PR7`, `ADV-PR10` | `IMPLEMENTATION READY`; re-hardened after accepted S2 validator delivery with archive summary/tool contract, S2 failure-class sync and source-controlled fixture family | `child-session-handoffs/adv-cas-s5-session-handoff.md` | Proposed: `openspec/changes/agent-delivery-visible-session-closeout-archive/` | S1 visible evidence fields implemented; S2 validator accepted; S5 may implement before S3 final live pass | `_specs/2026-05-10 Agent Delivery Visible Codex App Sessions ADV-CAS-S5 Closeout Archive Support.md`; `_specs/2026-05-10 Agent Delivery Visible Codex App Sessions Orchestration Pack.md`; `_specs/child-session-handoffs/adv-cas-s5-session-handoff.md`; `skills-repo/skills/spec-closeout/SKILL.md`; `docs/doc-workflow.md`; `skills-repo/tools/ArchiveVisibleCodexAppSession.cs`; `skills-repo/tools/ValidateVisibleCodexAppSessionEvidence.cs`; `tests/docworkflow-agent-delivery/e2e/fixtures/visible-app-session-closeout/**`; `tests/docworkflow-agent-delivery/e2e/validators/visible-session-closeout-summary.js`; `tests/docworkflow-agent-delivery/README.md`; `openspec/changes/agent-delivery-visible-session-closeout-archive/**` | Hardening gates: embedded JSON parse; `codex app-server --help`; `codex app-server generate-ts --out /tmp/codex-app-schema-adv-cas-s5`; `ValidateChildReadiness.cs`. Delivery gates: S5 archive fixtures, S2 visible evidence regression, source wording check, `git diff --check`; do not run `MD-E2E-5` | Reject final READY while visible sessions remain unarchived unless explicitly accepted; no live `thread/archive` call in hardening; S5 fixtures cover visible archive positive, already archived, headless not app-visible, queued no-thread, archive failure, post-archive proof failure, mixed child run and unarchived-visible negative | If live archive API is unavailable during delivery, fixture validation can still pass but closeout must record blocked or explicit retained-session acceptance before S3 pass | Start fresh `spec-change-delivery` for ADV-CAS-S5 |
...
| `ADV-PR5` | `ADV-CAS-S1`, `ADV-CAS-S2`, `ADV-CAS-S5` | partial | S1 freezes launcher evidence fields; S2 validates visible evidence and false positives; S5 still owns archive behavior. |
...
| `ADV-PR9` | `ADV-CAS-S5`, consumed by S3 | ready_for_delivery | S5 is implementation-ready; archive support must be delivered before MD-E2E-5 can pass. |
...
| complete | `ADV-CAS-S5` | Hardened archive support and closeout skill/tool contract after accepted S2 validator delivery. | Frozen default: closeout companion tool `ArchiveVisibleCodexAppSession.cs`; no Launcher archive mode required for S5. |
...
| S5 delivery | implementation | Can start after S2 acceptance; should complete before S3 live runner delivery. | `AgentDeliverySessionLauncher.cs` and S3 runner files stay read-only except for explicitly scoped handoff notes. | S5 delivery session. |
...
3. Deliver S5 before the final MD-E2E-5 live pass.
4. Harden S3 last, then deliver the MD-E2E-5 integration and run the live regression only in a later launcher/control session.
...
- Next Skill: S5 closeout archive hardening/delivery or S3 integration only after remaining dependencies are accepted.
