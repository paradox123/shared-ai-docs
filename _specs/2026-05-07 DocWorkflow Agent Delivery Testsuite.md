...
- Readiness Status: Ready for planning as Parent/Master Spec; not implementation-ready as a single change. `DWT-S0`, `DWT-S1`, `DWT-S2`, `DWT-S3` and `DWT-S4` are accepted and archived; `DWT-S5` is implementation-ready for its own delivery session after child-spec hardening. No runtime or Docker/container execution was used as DWT-S5 hardening acceptance proof.
...
| DWT-S5 | `_specs/2026-05-08 DocWorkflow Agent Delivery Testsuite DWT-S5 L3 Runtime Temp-Repo Delivery Pilot.md` | `DWT-PR3`, `DWT-PR4`, `DWT-PR5` | `IMPLEMENTATION READY` | `child-session-handoffs/dwt-s5-session-handoff.md` | `openspec/changes/docworkflow-agent-testsuite-dwt-s5-l3-runtime-temp-repo-delivery-pilot/`; canonical `openspec/specs/docworkflow-agent-delivery-testsuite/spec.md` | `DWT-S3` accepted real Promptfoo/Codex `ran-target` proof; synthetic runtime fixture to be created by DWT-S5 implementation | `_specs/2026-05-08 DocWorkflow Agent Delivery Testsuite DWT-S5 L3 Runtime Temp-Repo Delivery Pilot.md`; `_specs/2026-05-07 DocWorkflow Agent Delivery Testsuite.md`; `_specs/child-session-handoffs/dwt-s5-session-handoff.md`; `openspec/changes/docworkflow-agent-testsuite-dwt-s5-l3-runtime-temp-repo-delivery-pilot/**`; `openspec/specs/docworkflow-agent-delivery-testsuite/spec.md`; `tests/docworkflow-agent-delivery/l3/runtime-temp-repo/**`; `tests/docworkflow-agent-delivery/scripts/run-l3-runtime-temp-repo-checks.sh`; `tests/docworkflow-agent-delivery/README.md`; `tests/docworkflow-agent-delivery/testcases/tc2-single-child-delivery-next-handoff.md` | Hardening rehearsal passed: retained DWT-S3 JSON/manifest assertions; script syntax for L0/L1/S2/S3/S4; bundled Node; active DWT-S5 OpenSpec validate; canonical OpenSpec validate; DWT-S5 readiness validator; `git diff --check`. Delivery implementation evidence now includes `run-l3-runtime-temp-repo-checks.sh preflight --keep` and `all --keep`; final target proof remains blocked until Promptfoo/Codex runs. | Retained blocked-runtime DWT-S5 evidence `tests/docworkflow-agent-delivery/l3/runtime-temp-repo/evidence/2026-05-08-blocked-runtime/dwt-s5-l3-summary.json`; accepted predecessor evidence `tests/docworkflow-agent-delivery/l2/single-child-closeout/evidence/2026-05-08-ran-target/dwt-s3-l2-summary.json`; active OpenSpec ledger remains open | Runtime/container proof belongs to DWT-S5 delivery; Promptfoo/Codex unavailable or disabled must be `blocked_runtime`, not pass; no KI-fuer-KMU original repo may be named as runtime target or modified | Continue with `spec-change-delivery` or `spec-closeout` only for DWT-S5 after target Promptfoo/Codex proof is available; no descendant child is released |
...
`DWT-S0`, `DWT-S1`, `DWT-S2`, `DWT-S3` and `DWT-S4` are accepted and closed. `DWT-S2` supplies the accepted L2 parent-first orchestration proof with real Promptfoo/Codex `ran-target` evidence. `DWT-S3` supplies the accepted L2 single-child delivery/closeout proof with retained Promptfoo/Codex `ran-target` evidence. `DWT-S5` is now implementation-ready with its own child spec, allowed write-set, verification commands, evidence contract, active OpenSpec change and persisted handoff; delivery must still create the L3 runtime-temp-repo runner and retained evidence.
