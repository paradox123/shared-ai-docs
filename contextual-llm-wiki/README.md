...
Die festen Prüflisten liegen unter `scripts/upstream-tests.txt` und `scripts/integration-tests.txt`. Fehlende Dateien, nicht bestandene Tests und übersprungene/ausstehende Tests sperren die Eignung. Die Ergebnisdateien müssen jede Pflichtdatei mit tatsächlich ausgeführten Tests nachweisen; leere Dateien und per Upstream-Konfiguration ausgeschlossene Tests reichen nicht. Der Node-Prüflauf entfernt geerbte `NODE_OPTIONS`, damit Testfilter keine Pflichtszenarien auslassen. Die Integration verwendet echte Compiler-/QMD-Abläufe mit künstlichen Fachquellen, separaten Datenbanken und einem deterministischen lokalen Modellprovider. `fixtures/` enthält ausschließlich diese Testbestände. Aktive Wiki-Daten und Pflegeautomation werden nicht verwendet. Der öffentliche Aufruf `./wiki update --candidate PATH` aktiviert einen qualifizierten Kandidaten; `./wiki release-status` zeigt die tatsächlich ausgewählte Runtime. Ablauf und Wiederherstellung stehen in der [Betriebsanleitung](OPERATIONS.md#geprüfte-releases-lokal-aktivieren). Regelmäßige Release-Erkennung folgt in Ticket 03.
