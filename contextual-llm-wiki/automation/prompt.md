Maintain the configured general DanielsVault LLM wiki, then the single QMD index and embeddings.
...
Execution:
1. Follow the QMD Scheduled Index Maintenance runtime/database preflight. Load Homebrew shellenv where available, resolve QMD_BIN and Node, and check the QMD database directory is writable. If runtime or permissions are unavailable, record the blocker and stop; do not repair installations or macOS TCC.
2. Run this repository-owned helper once from the local checkout. The general config is the full source inventory, not the acceptance fixture. In the same shell as QMD_BIN resolution, use:
...
LLMWIKI_PROVIDER=codex-agent python3 "$WIKI_HOME/scripts/maintain-index.py" \
  --config "$WIKI_HOME/.local/general.json" \
...
- Only `.local/general.json` is selected. Do not add private scope, substitute acceptance configs, reduce source coverage, create other schedulers/watchers, commit, push or merge.
