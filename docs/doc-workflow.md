...
11. `spec-closeout` archiviert alle sichtbaren Codex-App-Sessions, die fuer die jeweilige Child-Spec-Bearbeitung geoeffnet wurden, bevor Child-Closeout `READY` melden darf. Headless- und Queue-Evidence erhalten stattdessen explizite Archivstatus wie `not_app_visible_not_archived` oder `no_thread_created`.
