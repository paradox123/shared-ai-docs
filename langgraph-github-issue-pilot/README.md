...
export PILOT_CODEX_INTERVENTION_SURFACE="stable-app-server"
...
`PILOT_CODEX_INTERVENTION_SURFACE=stable-app-server` is required in production. Ordinary implementation, review, and repair workers continue to use isolated `codex exec` processes. Only a schema-valid, durably persisted human intervention is mirrored into the Codex App. That adapter uses the documented stable stdio thread methods with `experimentalApi: false`; experimental methods or transports, `exec-server`, and private Codex storage are not fallback paths. If those stable methods are unavailable, public workflow read-back reports `delivery_blocked` and retains the request without continuing autonomously.

The implementer runs non-interactively with the packaged node policy (`gpt-5.6-terra`, `xhigh`, `workspace-write`), a run-owned `codex/run-<run-id>` branch, issue-type skill hashes, and the packaged worker-result schema. The schema-constrained `--output-last-message` file is the authoritative result channel and is validated independently from JSONL diagnostics. A malformed diagnostic line becomes a bounded `pilot.diagnostic_parse_failed` event and cannot discard a schema-valid completed, blocked, or intervention result. Genuine process/result failures retain their safely parsed redacted diagnostics and one stable failure code such as `process_timeout`, `final_result_missing`, or `schema_validation_failed`; raw malformed output and arbitrary exception text are not persisted. Invalid or failed results are not promoted to a pull request. The implementer is not responsible for `git add` or `git commit`: the controller-owned publisher performs staging and commit after successful evidence qualification, so linked-worktree Git-metadata restrictions are not implementation blockers. Repair assignments use separate versioned contracts, the same worker port, skills, worktree, and branch, plus a hard per-initial-review limit of three numbered rounds. Rounds one and two use Terra unless an allowed material or structured escalation applies; round three records `final_repair_round` and uses `gpt-5.6-sol`, `xhigh`, with the same bounded implementer write root.
...
Publication requires exactly one passing evidence item per acceptance criterion. REST items require request, relevant response, and business read-back; UI items require executed interaction and a decisive screenshot reference; document-only items require direct rendered-document read-back; recovery and idempotency require restart/repetition plus public read-back; negative gates require rejection plus proof that the forbidden side effect is absent; background work requires the eventually observable business result. Builds, process/container starts, healthchecks, naked status codes, enqueue success, static starting screenshots, and log claims are rejected as sole evidence. Correlated logs remain supplemental.

Before creating a new run worktree, the Git adapter fetches the configured branch from `origin`, resolves the fetched remote ref to one immutable commit SHA, creates the run branch from that SHA, and stores the SHA as the worktree's durable comparison base. A pilot-owned local base ref preserves that identity across the narrow crash window before SQLite preparation is durable. Recovery adopts an existing run worktree and its original SHA without moving it when the remote branch advances. Before publication, worker evidence and diagnostics redact the configured GitHub/webhook secrets plus recognizable tokens, authorization values, credential fields, and email addresses. The outgoing diff is scanned for the same material and fails closed rather than rewriting source. The Git adapter uses the worktree's `origin`, creates `Implement issue #<number>` only when the worker left uncommitted changes, pushes the explicit run branch, and supplies the observed head SHA to the PR renderer. Production readiness requires a repository-local GitHub noreply author identity so GitHub email-privacy protection cannot reject that push. A branch with no implementation commit is not published.
...
After startup completes, there is no periodic GitHub polling. A local heartbeat updates SQLite once per minute, delivers persisted interventions to their single Codex App task, reads the first later user answer, and resumes the same LangGraph checkpoint under the stored operation identity. It also records liveness again on graceful shutdown so a later boot can measure the last durable proof of life. Workflow read-back includes the intervention request/session/answer/application lifecycle plus the latest boot reconciliation ID, threshold outcome, status, prior/current liveness timestamps, measured offline seconds, and discovered/accepted/deduplicated command counts. These records exclude webhook and GitHub bodies, tokens, email addresses, and arbitrary external exception text.
...
The same guide defines the final live activation gate: `pilotctl live-readiness` performs a read-only GitHub/profile/backlog check, `pilotctl ensure-live-labels` explicitly bootstraps only missing workflow label definitions, and `pilotctl capture-live-evidence` validates one verified current head and writes a bounded private correlation manifest. These commands reuse the production adapter profile; they do not add scheduler paths or bypass GitHub events.
