...
| `--live-subagents` | Use live Codex subagents when the user explicitly wants delegated/parallel council seats |
...
- `live_agents_authorized`: false unless the user explicitly asks for live/parallel/delegated subagents, or passes `--live-subagents`
...
Run provider detection before routing when the script exists:

```bash
bash ~/.codex/skills/council/scripts/detect-providers.sh
```

Use the JSON `providers[]` entries whose `available` value is true. In Codex, do not assume Anthropic is native; only route to providers reported as available by the detector or explicitly configured by the user.

### Step 3: Run Restatement Gate (Parallel)

Default Codex behavior is a local simulated council: read the selected persona files and generate clearly labeled `[Simulated]` seat outputs without spawning subagents. This keeps `/council` compatible with Codex hosts where subagent spawning requires an explicit user request.

If `live_agents_authorized=true`, spawn one Codex sub-agent per selected member with `spawn_agent`, `fork_context=true`. Do not request Anthropic-specific subagent types in Codex; pass the persona file path/content in the prompt and use the host's default subagent model unless the user explicitly requested a model.
...
Wait with `spawn_timeout_ms`. If a live seat fails or times out:
...
For all CLI archetypes, build the full prompt in a temporary file using a quoted heredoc or stdin. Never inline problem text directly in a shell command.

**`codex_exec` (OpenAI via Codex CLI)** — dispatch via subprocess:

- Inline the member identity/grounding/output sections from `~/.codex/skills/council/agents/council-{name}.md`.
- Run read-only and non-interactive:
...
{full prompt}
COUNCIL_PROMPT_EOF
codex exec -m "{model}" --sandbox read-only - < "$PROMPT_FILE"
rm -f "$PROMPT_FILE"
```

- Capture the final response as the member output. Empty output, non-zero exit, or timeout marks the seat `degraded`.

**`gemini_cli` (Google via Gemini CLI)** — dispatch via subprocess when `gemini` is available:
...
{full prompt}
COUNCIL_PROMPT_EOF
gemini -m "{model}" -p "$(cat "$PROMPT_FILE")"
rm -f "$PROMPT_FILE"
```

**`ollama_run` (local Ollama)** — dispatch via subprocess when `ollama` is available:
...
- Extract `.choices[0].message.content`. If empty or non-2xx, mark the seat `degraded` and apply the standard fallback (simulation, or `codex_exec` when available).
...
- Run headless and read-only using the same temp-file prompt pattern: `cursor-agent -p --mode ask --model {model} --output-format text "$(cat "$PROMPT_FILE")"`.
...
- Counts as a single provider for spread. Because Cursor can serve `claude-*` models, prefer cross-family models (`gpt-*`, `gemini-*`, `grok-*`) when the goal is provider diversity. Verify live IDs with `cursor-agent --list-models`.

### Step 4: Deliberation Rounds

Keep the same seat state for all rounds. For live subagents, continue with `send_input`; for simulated or external-provider seats, carry forward the prior-round transcript explicitly in each prompt.
...
1. For simulated seats, produce outputs locally from persona instructions and prior round context.
2. For live or external seats, send prompts to all `live` seats in parallel where the host/tooling allows it.
3. Wait using `round_timeout_ms`.
4. For each missing live response, retry `send_input` up to `retry_attempts` with a stricter prompt: "Respond now in <= {word_limit} words."
5. If still missing, move seat to `degraded` and generate `[Simulated]` output from persona instructions plus prior round context.
...
1. **Explicit override**: `--chairman <name>` was passed (provider tag — `openai`, `google`, `ollama`, `nvidia_nim`, `cursor_cli` — or a model alias; `anthropic` is valid only if explicitly configured and available).
...
If `spawn_agent` is unavailable, live subagents were not explicitly authorized, or too many seats fail, run a local simulated council:
