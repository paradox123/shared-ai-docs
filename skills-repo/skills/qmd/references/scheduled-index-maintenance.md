...
On this Mac the existing QMD Node runtime is `/opt/homebrew/opt/node@22/bin`; the saved prompt adds it explicitly after Homebrew setup and verifies availability. Do not install a replacement when it is missing.

For `update-qmd-index-daily`, follow the repository's [canonical saved prompt](../../../../contextual-llm-wiki/automation-prompt.md). It invokes `contextual-llm-wiki/scripts/run-maintenance.py` once after this preflight. The runner owns one `maintain-index.py` process: reconciliation, independent original-source indexing, bounded wiki maintain/status/lint, then safe global QMD update/embed/status. Do not execute the generic Rule 4 commands in addition to that helper.

The common production config includes all eight original repositories and recursive Meetings/Projects including Projects/Private. Sources and source configuration remain read-only. [Operations](../../../../contextual-llm-wiki/OPERATIONS.md) documents deployment, rollback and manual use.

Retain the exact first artifact path and execution handle. Take the run's time budget, termination grace, observation margin, stall threshold and termination allowance from the authorized invocation in the saved prompt; take extraction capacity and concurrency from its named production configuration. The repository's canonical prompt and Operations document explain that contract. Do not copy those changing production values into this skill or substitute helper defaults. If the saved prompt and repository contract disagree, report the discrepancy without silently editing the automation or enlarging the run's authorized limits.

Observe the same handle with waits no longer than 60 seconds until it completes or the absolute elapsed-time bound is reached. Preserve that deadline across context switches; the number of polls is not a work budget. At the configured stall threshold use the runner's one durable-progress diagnosis. Never renew the allowance or launch a second collector to recover output. Finite limits are not throughput guarantees.

Progress lives at `<artifacts>/maintenance/progress.json` and its named detail; process custody at `process.json`. Completion lives at `observation.json`, `helper.exitcode`, and `maintenance/report.json`. Full success requires both exits 0, explicit `ok:true`, and no pending/remaining work. At the observation boundary or missing/malformed report, read one `diagnosis.json`, bounded progress and at most 4000 stderr characters, record failure/incompleteness and stop. A living process is not progress; unknown/live ownership or custody markers block another run and are not permission to delete locks or kill foreign processes.

Memory separates current source-index result, daily pending/overdue work, initial pending/unextracted backlog, capacity, wiki lastCompleted/global completion and outer QMD update/embed/status. Missing dimensions stay unknown. Preserve separate open acceptance for the first actual scheduled execution and full multi-day import followed by no-op. Neither configuration nor a manual productive run proves either event.
