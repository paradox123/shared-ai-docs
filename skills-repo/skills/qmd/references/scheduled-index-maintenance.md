...
Retain the exact first artifact path and execution handle. Limits: helper 1200 seconds, termination grace 10 seconds, observation margin 30 seconds (1240 total), one durable-progress diagnosis after 180 seconds without progress, at most 11 further seconds for owned-helper termination. Maximum 20 new extractions per helper, configured concurrency 8. These are finite safety limits, not throughput guarantees. Poll the same handle at most 22 times with waits no longer than 60 seconds; never renew the allowance or launch a second collector to recover output.
