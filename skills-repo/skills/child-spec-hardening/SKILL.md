...
When the verdict is `IMPLEMENTATION READY` or `READY WITH NON-BLOCKING NOTES`, always produce a persisted Child Session Handoff for the implementation run using the shared workflow template: parent path, stable child id, child spec path, child index/queue path, handoff file path, target repository / working directory, Codex session/log metadata when available, resolver-backed session evidence or explicit `legacy_reconstructed` marker when historical, next mode/skill, current verdict, scope summary, non-goals, enforceable allowed write-set, shared/read-only files, verification lifecycle, evidence/OpenSpec, retained evidence, open non-blocking notes, and whether a fresh session is recommended. Link the same handoff from the Child Index. Then create or require Agent Delivery Session Launch/Queue Evidence via `skills-repo/tools/AgentDeliverySessionLauncher.cs --mode queue --handoff <handoff> --target-id <child> --agent codex --out _specs/agent-delivery-session-launches`, and run or require `WorkflowDoctor.cs --phase evidence-resolution --mode launcher_only --claim-level queued ...` for the resulting artifacts. If the workflow requires controller-backed visible sessions, name `AgentDeliveryVisibleSessionController.cs` as the external orchestrator and require resolver mode `controller_visible_multi_session`. For large Parent/Child work, prefer a fresh implementation session per child.
...
- Do not treat a fresh implementation session as queued, launched, or controller-backed unless the canonical resolver returns `pass` for the relevant mode and claim level.
- Do not use a semantic `SessionId` as the only future session proof; require resolver-backed evidence or explicit `legacy_reconstructed` source/date for historical pre-launcher transitions.
