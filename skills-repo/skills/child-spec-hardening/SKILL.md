...
- Do not treat a fresh implementation session as queued or launched unless `_specs/agent-delivery-session-launches/**/launch-request.json` and `evidence.json` contain matching Agent Delivery Session Launch/Queue Evidence with status `queued` or `launched`.
