...
- After creating or updating the leading handoff, create Agent Delivery Session Launch/Queue Evidence with `skills-repo/tools/AgentDeliverySessionLauncher.cs --mode queue` or document the explicit manual/blocked/failed reason. For controller-backed visible multi-session workflows, follow `docs/doc-workflow.md`: use `AgentDeliveryVisibleSessionController.cs` as the external orchestrator and treat the launcher as the per-session primitive.
- When deciding whether a launch/queue/controller claim is acceptable, run or require the canonical resolver: `dotnet run skills-repo/tools/WorkflowDoctor.cs -- --phase evidence-resolution --mode <launcher_only|controller_visible_multi_session> ...`. A resolver verdict of `not_ready` or `fail` blocks next-child release. Surface its `blockers` and `recommended_next_action` instead of restating the detailed evidence matrix here.
