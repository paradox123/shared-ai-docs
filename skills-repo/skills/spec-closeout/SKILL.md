...
7. Agent Delivery Session Launch/Queue Evidence: when closeout releases a next leading handoff, run or require `skills-repo/tools/AgentDeliverySessionLauncher.cs --mode queue --handoff <handoff> --target-id <child> --agent codex --out _specs/agent-delivery-session-launches`. Then run or require `WorkflowDoctor.cs --phase evidence-resolution --mode launcher_only --claim-level queued ...`. For controller-backed visible multi-session workflows, release through `AgentDeliveryVisibleSessionController.cs` and require resolver mode `controller_visible_multi_session`. Resolver `not_ready` or `fail` prevents advancing the next child; surface `blockers` and `recommended_next_action`.
8. Codex-App Session Archive: before final child `READY`, validate closeout archive evidence with `WorkflowDoctor.cs --phase evidence-resolution --mode closeout_archive --archive-summary <summary>`. Use `ArchiveVisibleCodexAppSession.cs` to create or validate archive summaries when needed; live `thread/archive` remains explicit opt-in. Resolver `not_ready` or `fail` blocks `READY`.
