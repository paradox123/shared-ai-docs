...
- Treat every command shown in an automation prompt as an executable contract. Spell the canonical path for each helper that is not guaranteed to be on `PATH`, or assign that path once and reuse it; never substitute `...` for the program path or refer to a helper only by basename. A prompt that must run before skills can be loaded cannot rely on later skill discovery to locate its bootstrap helper.
...
- Define the report's action contract alongside constraints and no-change behavior. Separate a technical failure, incomplete work, and a decision actually needed from the user. For a blocker, state the observed failed step, what completed or remains unverified, and who owns the next concrete action. Do not infer a root cause from a generic error label. If no user decision is needed, say so; reserve `needs user` for a specific missing fact, choice, or authorization and present an answerable question through an available reply surface. Internal session IDs, digests, artifact paths, and error codes are evidence, not user instructions. Preserve privacy limits; ask only for the minimum safe decision context, and do not invite another run unless retry is authorized and the failed prerequisite is resolved or a documented retry condition applies.
...
7. Add or run a small verifier for:
   ...
   Check the resulting human report as well as the helper result: a technical failure must not invent a user decision, and a real missing decision must name the requested answer and how to provide it. This does not authorize automatic repairs or retries.
...
12. When an automation prompt or helper needs `$CODEX_HOME`, choose one canonical value with `CODEX_HOME_RESOLVED="${CODEX_HOME:-$HOME/.codex}"` instead of probing multiple candidate directories. Shell variables do not persist across tool calls: redeclare that same normalization at the start of every separate Codex-state shell, or pass the resolved path as an explicit quoted argument. Never compensate with a hard-coded resolved home path.
