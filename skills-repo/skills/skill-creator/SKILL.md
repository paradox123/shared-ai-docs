...
└── Bundled Resources (optional)
    ├── scripts/          - Executable code (C#/Python/Bash/etc.)
...
- Generate deterministically by passing the values as `--interface key=value` to `python3 scripts/generate_openai_yaml.py` or `python3 scripts/init_skill.py`
...
- **Example**: `scripts/rotate_pdf.cs` or `scripts/rotate_pdf.py` for PDF rotation tasks
...
python3 scripts/init_skill.py my-skill --path ~/work/skills --resources scripts --examples
```
