...
`session_index.updated_at` is a selection timestamp, not a reliable completion timestamp. A rollout can start before `review_window_end`, remain open at that boundary, and append later without receiving a newer index row. Keep one top-level memory field named `Carry-forward sessions:` containing the resolver's compact JSON array of `{id, line_count, last_activity_at}` checkpoints, or `[]`. The resolver always reselects those ids even when their index rows are older than the next cutoff. If Codex has pruned a carried id from the current index, the resolver synthesizes only the minimal routing row from that trusted checkpoint and still resolves the rollout by verified exact id; a missing or ambiguous rollout remains unsafe. Line counts prevent same-timestamp appended records from being lost.
...
For an ad-hoc review without automation memory, `--recent` still controls only how many captured index rows are echoed in the resolver output; it never establishes the review cutoff. Pass an explicitly trusted boundary with `--prompt-last-run`. If no trusted boundary exists, use bounded task/thread inspection or ask the user for one instead of invoking the resolver with `--recent` alone.
...
For every exact-evidence check, never print raw JSONL records with `sed`, `head`, `tail`, an inline JSONL parser, or an equivalent whole-record reader. First rerun the persisted manifest through the helper's structural mode with every exact positive id explicitly allowlisted; repeat `--session-id` when needed:
...
Only when the structural projection and compact summary show that a finding genuinely needs one concrete tool call, rerun the same persisted manifest with the exact structural line. Use the legacy unindexed selector for a direct call or a custom recorder wrapper containing exactly one nested call. For a custom `exec` wrapper containing several nested calls, append the one-based nested-call index in source order. Tool-shaped text inside JavaScript strings, comments, or regex literals does not consume an index; real calls inside template `${...}` expressions do. Repeat both flags when several positively identified calls are required:
...
python3 ~/Documents/DanielsVault/_shared/shared-ai-docs/skills-repo/skills/improve-skills/scripts/extract_codex_session_evidence.py \
  --manifest "$RESOLVER_MANIFEST" \
  --session-id <exact-session-id> \
  --tool-call-projection <exact-session-id>=<exact-tool-call-line>#<one-based-nested-index>
```

Tool-call projection requires matching exact `--session-id` filters, stays inside each resolver-advertised range, and fails closed for unknown, unresolved, out-of-range, duplicate, or non-tool lines. An unindexed selector fails when a custom wrapper contains multiple nested calls; indexed selectors fail for direct calls and for missing, non-integer, non-positive, or out-of-range indices. Wrappers using optional chaining, bracket property access, or another indirect `tools` reference are ambiguous and fail before projection or result pairing instead of letting a remaining direct call appear unique. The result includes `nested_call_index` (`null` for direct calls and recorder-only wrappers) so repeated projections from one structural line remain distinguishable. It cannot be combined with path selectors, structural projection, or clone boundary/suffix modes.

The projection emits only the normalized tool name plus a sanitized, capped command and cwd for `exec_command`; home paths become `~/...`, common credential assignments and flags are redacted, and arbitrary arguments for other tools are never rendered. A direct function call with one uniquely matching call-id result may contribute only a fixed status label and integer exit code. A custom recorder result belongs to the outer wrapper and may contain arbitrary recorder-emitted JSON, so custom projections never populate `paired_status` or `paired_exit_code`. This includes nested projections even when the wrapper source contains only one nested call. Do not infer an inner command's exit from wrapper completion or wrapper-emitted JSON. Tool output/body, neighboring records, thread names, reasoning, and unselected payloads are never emitted. If the selected call cannot be parsed safely, its optional command, cwd, status, or exit-code field remains `null`; do not fall back to `jq`, raw JSONL readers, inline parsers, or `read_thread` for command detail.
