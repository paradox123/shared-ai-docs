...
usage() {
  cat <<'USAGE'
Usage:
  run-l2-single-child-closeout-checks.sh [all|agent|fallback|validate-output|closeout|style|telemetry] [--run-dir DIR] [--keep] [--output-bundle DIR]

Runs DWT-S3 L2 single-child delivery and closeout checks against source-controlled
fixtures. The runner writes evidence/dwt-s3-l2-summary.json in an isolated run
dir. Fallback artifact mode can validate deterministic contracts or report a
blocked agent path, but it is not accepted DWT-S3 proof without
...
L2_DIR="$SUITE_DIR/l2/single-child-closeout"
FIXTURE_SRC="$L2_DIR/fixtures"
VALIDATOR="$L2_DIR/validators/single-child-closeout-validator.js"
PROMPTFOO_CONFIG="$L2_DIR/promptfooconfig.yaml"
PROMPTFOO_VERSION="${DWT_S3_PROMPTFOO_VERSION:-0.121.9}"
...
case "$selector" in
  all|agent|fallback|validate-output|closeout|style|telemetry) ;;
  *)
...
created_run_dir=0
if [ -z "$run_dir" ]; then
  run_dir="$(mktemp -d "${TMPDIR:-/tmp}/docworkflow-agent-delivery-l2-single-child-closeout.XXXXXX")"
  created_run_dir=1
else
  mkdir -p "$run_dir"
fi
...
case "$selector" in
  agent)
    ...
    ;;
  all)
    if [ "${DWT_S3_ENABLE_AGENT:-0}" = "1" ]; then
      run_promptfoo_agent=1
    fi
    ;;
esac
...
if [ "$run_promptfoo_agent" -eq 1 ]; then
  ...
  export DWT_S3_FIXTURE_WORKSPACE="$run_dir/agent-workspace"
  export DWT_S3_PROMPTFOO_EVAL_JSON="$promptfoo_eval_json"
  export DWT_S3_PROMPTFOO_EVAL_LOG="$promptfoo_eval_log"
  export DWT_S3_PROMPTFOO_VERSION="$PROMPTFOO_VERSION"
  export DWT_S3_PROMPTFOO_AUTH_STATUS="$promptfoo_auth_status"
  mkdir -p "$DWT_S3_FIXTURE_WORKSPACE"
  ...
  export DWT_S3_PROMPTFOO_EXIT_STATUS="$promptfoo_status"
fi
