...
Write the first workflow value, `1`, to `tests/docworkflow-agent-delivery/e2e/session-workflow-live/20260511T151733Z-post-spec-regression/target/output/count.txt` from a launcher-created child Codex session.

## Review Control Surface

- Spec variant: child delivery regression spec.
- Goldstandard status: implementation-ready direct regression ledger.
- Goal: prove `RADW2-PR4`.
...
- Covers `RADW2-PR4`: Child 1 writes `1` as the first line of `target/output/count.txt`.

## Parent Scope Conformance

| Parent Requirement | Child Claim | Conformance | Action |
|---|---|---|---|
| RADW2-PR4 | Write only `1` as the first line. | preserves | Deliver in RADW2-C1 only. |
| RADW2-PR5 through RADW2-PR8 | Later children write later values. | defers_to_child | Do not write other child values. |
| RADW2-PR11 | Parent closeout consumes this child evidence. | preserves | Write child closeout JSON. |

## In Scope

- Ensure `target/output/` exists.
- If `count.txt` exists with non-empty content, stop as `NOT READY` and write `delivery-evidence/radw2-c1/not-ready.md`.
- Write exactly `1\n` to `count.txt`.
- Write `delivery-evidence/radw2-c1/delivery.json`.
- Write `closeout/children/radw2-c1.json`.
...
- `delivery-evidence/radw2-c1/delivery.json` contains `final_status: ran-target`.
- `closeout/children/radw2-c1.json` contains `final_status: ran-target` and `closeout_status: closed`.
...
node -e "const fs=require('fs'); const s=JSON.parse(fs.readFileSync('tests/docworkflow-agent-delivery/e2e/session-workflow-live/20260511T151733Z-post-spec-regression/closeout/children/radw2-c1.json','utf8')); if (s.final_status !== 'ran-target' || s.closeout_status !== 'closed') process.exit(1);"
```
...
- `delivery-evidence/radw2-c1/delivery.json`
- `closeout/children/radw2-c1.json`
